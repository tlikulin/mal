...
fn main() -> io::Result<()> {
    REPL_ENV.with(|repl_env| {
        ...
        if let Some(argv1) = cmd_args.next() {
            ...
            repl_env.set("*ARGV*".to_string(), new_list(argv));
        ...
        // interactive
        let _startup_msg = rep(r#"(println (str "Mal [" *host-language* "]"))"#, repl_env);
        loop {
            print!("user> ");
            io::stdout().flush()?;
...
fn quasiquote(ast: MalType) -> MalResult {
    ...
    match ast {
        ...
        MalType::List(seq, ..) | MalType::Vector(seq, ..) => {
            ...
            let result = new_list(result);
            if was_vector {
                Ok(new_list(vec![MalType::Symbol("vec".to_string()), result]))
            } else {
                Ok(result)
            }
        }
        ...
        MalType::HashMap(..) | MalType::Symbol(..) => {
            Ok(new_list(vec![MalType::Symbol("quote".to_string()), ast]))
        }
